...
export const paymentService = {
  ...
  processPayment: async (method, amount, cashReceived) => {
    ...
    if (method.id === 'efectivo' && cashReceived < amount) {
    ...
    return {
      ...
      tipoPago: method.nombre,
      monto: amount,
      efectivoRecibido: cashReceived || 0,
      cambio: cashReceived ? cashReceived - amount : 0,
      referencia: `${method.id.toUpperCase()}-${Date.now()}`,
      fecha: new Date().toISOString(),
    };
  },
};
